...
def discord_bot():
                        ...
                        if coolDownOK:
                            ...
                            if itemType == 'single':
                                ...
                                if vaultRentals is not None:
                                    ...
                                    if availableVaults is None or len(availableVaults) == 0:
    ...
    async def orderStatusWatcher():
        while True:
            try:
                ...
                shopCur = shopCon.cursor()
                shopCur.execute(
                    "SELECT id, order_number, itemid, in_process, completed, refunded, order_date, last_attempt, completed_date, discordMessageID, discordChannelID "
                    "FROM order_processing "
                    "WHERE orderCompleteNoticeSent IS False "
                    "AND discordMessageID IS NOT NULL")
                changedOrders = shopCur.fetchall()
                if changedOrders is not None:
                    for order in changedOrders:
                        ...
                        elif completed is True and refunded is False:
                            status = "Complete"
    ...
    async def refundAll(channelID):
        ...
        dbCur = dbCon.cursor()
        dbCur.execute(
            "SELECT DISTINCT order_number, in_process, completed, refunded FROM order_processing WHERE completed = False AND in_process = False AND refunded = False")
    ...
    async def permissionWatcher():
        while True:
            ...
            if privelegedRoles is None or len(privelegedRoles) == 0:
                    ...
                    if rentals is not None:
                        ...
                        for rental in rentals:
                            ...
                            if lastAccessed is not None:
                                ...
                                if inUse is True and now > exitvaultExpiration:
                                    dbCur.execute(
                                        "UPDATE {server}_vault_rentals SET inUse = False WHERE vaultname = '{name}'".format(
                                            server=server[1], name=rentalname))
                                    dbCon.commit()
                            embedvar.add_field(name="{} \nRented by: {}".format(rentalname, renterDiscord),
    ...
    async def exitVault(senderID, channelID):
        ...
        if result is not None:
            ...
            if vaultDetails is not None:
                ...
                if homeLocation is not None and inUse is True:
