...
def discord_bot():
                    ...
                    try:
                        ...
                        if messages != None:
                            ...
                            for message in messages:
                                ...
                                if "#0" in destChannel:
                                    destChannel = destChannel.replace("#0","")
        ...
        while True:
            ...
            try:
                ...
                for category in categories:
                    cat = category[0]
                    embedvar = discord.Embed(title=cat, color=discord.Color.blue())
                    embedvar.set_footer(
                        text="Chakram Gaming 2024")
                    shopCur.execute(
                    ...
                    if int(senderCurrency) >= int(itemprice):
                        ...
                        if coolDownOK:
                            ...
                            if itemType == 'single':
                                ...
                                shopCon.commit()
                                order_placed = True
                            elif itemType == 'knowledge':
                                print("item type is knowledge, placing order")
                                shopCur.execute(
                                    "INSERT INTO order_processing (order_number, order_value, itemid, itemType, itemcount, purchaser_platformid, purchaser_steamid, in_process, completed, refunded, order_date, last_attempt) values (?,?,?,?,?,?,?,?,?,?,?,?)",
                                    (order_number, itemprice, itemid, itemType, itemcount, platformid, steamid, False,
                                     False, False, order_date, last_attempt))
        ...
        while True:
            ...
            if servers != None:
                for server in servers:
                    ...
                    footer = "Current Wanted Players can be tracked at https://map.chakram.co"
                    embedvar.set_footer(text=footer)
                    await channel.send(embed=embedvar)
                    ...
                    embedvar = discord.Embed(title='Kill Streaks', color=discord.Color.gold(),
                                             url="https://map.chakram.co".format(servermapurl=serverurl))
    ...
    async def goToVault(senderID, channelID):
        ...
        if result != None:
            ...
            if vaultDetails != None:
                ...
                if dayofWeek != 2 or dayofWeek != 4 or dayofWeek != 6:
    ...
    async def on_ready():
        ...
        if (config.Server_Prison_Enabled == 'True'):
            print(f"Server_Prison_Enabled is {config.Server_Prison_Enabled}")
    ...
    async def on_message(message):
        ...
        if message.content.startswith('!gift'):
            senderDiscordID = message.author.name
