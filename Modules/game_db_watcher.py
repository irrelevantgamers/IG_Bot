...
def watch_game_db():
    while True:
        ...
        dbCur.execute("SELECT cellName, prisoner, sentenceTime, sentenceLength, assignedPlayerPlatformID FROM {server}_jail_info".format(server=config.Server_Name))
        ...
        if result != None:
            for result in result:
                ...
                if prisoner == None:
                    ...
                    if remaining <= 0:
                        TimeLeft = 0
                        print("release prisoner {assignedPlayerPlatformID} from cell {cellName}".format(assignedPlayerPlatformID=assignedPlayerPlatformID, cellName=cellName))
                        #get server info
                        ...
                        #release prisoner
                        dbCur.execute("INSERT INTO {server}_teleport_requests (player, dstlocation, platformid) VALUES (?,?,?)".format(server=config.Server_Name), (prisoner, Prison_Exit_Coordinates, assignedPlayerPlatformID))
                        dbCur.execute("UPDATE {server}_jail_info SET prisoner = NULL, assignedPlayerPlatformID =NULL, sentenceTime = NULL, sentenceLength = NULL WHERE cellName = ?".format(server=config.Server_Name), (cellName,))
                        dbCon.commit()
                                

            #check if prisoners escaped
            print("Checking if prisoners escaped")
            dbCur.execute("SELECT cellName, prisoner, sentenceTime, sentenceLength, assignedPlayerPlatformID, spawnLocation FROM {server}_jail_info".format(server=config.Server_Name))
            result = dbCur.fetchall()
            if result != None:
                for result in result:
                    cellName = result[0]
                    ...
                    if prisoner == None:
                        ...
                        cursor.execute(f"SELECT c.id, c.playerid, c.char_name, a.user as PlatformID FROM characters c LEFT JOIN account a on c.playerid = a.id WHERE a.user =?", (assignedPlayerPlatformID, ))
                        ...
                        dbCur.execute("SELECT prison_min_x, prison_min_y, prison_max_x, prison_max_y FROM servers WHERE serverName =?",(config.Server_Name, ))
                        info = dbCur.fetchone()
                        if info != None:
                            prison_min_x = info[0]
                            prison_min_y = info[1]
                            ...
                            if playerX < prison_min_x or playerX > prison_max_x or playerY < prison_min_y or playerY > prison_max_y:
                                ...
                                #move them back
                                dbCur.execute("SELECT rcon_host, rcon_port, rcon_pass FROM servers WHERE serverName =?",(config.Server_Name, ))
                                serverInfo = dbCur.fetchone()
                                if serverInfo != None:
                                    rcon_host = serverInfo[0]
                                    rcon_port = serverInfo[1]
                                    rcon_pass = serverInfo[2]
                                #release prisoner
                                dbCur.execute("INSERT INTO {server}_teleport_requests (player, dstlocation, platformid) VALUES (?,?,?)".format(server=config.Server_Name), (prisoner, spawnLocation, assignedPlayerPlatformID))
                                dbCon.commit()
