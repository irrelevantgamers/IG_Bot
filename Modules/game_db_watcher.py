...
def watch_game_db():
    while True:
        ...
        gamedbCur.execute(
            "select DISTINCT  g.guildid, g.name, (select count() from building_instances where object_id in "
            "(select object_id from buildings where owner_id = g.guildId)) as count "
            "FROM guilds as g "
            "LEFT JOIN buildings as b ON b.owner_id = g.guildId "
            "ORDER BY count DESC")
        ...
        gamedbCur.execute(
            "SELECT g.guildid, g.name, count(*) as InventoryCount FROM Detailed_structure_inventory as dsi "
        ...
        if invdbrows is not None and len(invdbrows) > 0:
