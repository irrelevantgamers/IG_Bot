    ...
    while True:
        ...
        if servers != None:
            for server in servers:
                ...
                dbCur.execute("SELECT player, platformid, dstLocation, attempts FROM {servername}_teleport_requests WHERE completed =False AND lastAttempt <= ?".format(servername=server[1]), (eligibleProcessTime,))
                ...
                if result != None:
                    for request in result:
                        ...
                        dbCur.execute("SELECT rcon_host, rcon_port, rcon_pass FROM servers WHERE serverName =?",(server[1], ))
                        ...
                        if serverInfo != None:
                            ...
                            dbCur.execute("SELECT platformid FROM {servername}_currentusers WHERE platformid =?".format(servername=server[1]), (platformid,))
                            ...
                            if result != None:
                                ...
                                if conid != None:
                                    ...
                                    while success == 0 and attempts <= 5:
                                        try:
                                            ...
                                            dbCur.execute("UPDATE {servername}_teleport_requests SET completed =?, attempts =? WHERE platformid =?".format(servername=server[1]), (True, tpattempts, platformid))
                                            dbCon.commit()
                                            print("Teleported {player} to {dstLocation}".format(player=player, dstLocation=dstLocation))
                                ...
                                tpattempts = tpattempts + 1
                                dbCur.execute("UPDATE {servername}_teleport_requests SET attempts =? WHERE platformid =?".format(servername=server[1]), (tpattempts, platformid))
                                dbCon.commit()
                        if tpattempts >= 6:
                            dbCur.execute("UPDATE {servername}_teleport_requests SET completed =True WHERE platformid =?".format(servername=server[1]), (platformid,))
                            dbCon.commit()
