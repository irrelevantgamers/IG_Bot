...
def pay_users():
    ...
    if servers is not None:
        for server in servers:
            ...
            if currentUsers is not None and currentUsers != '':
                for user in currentUsers:
                    platformID = user[0]
                    dbCur.execute(
                        "select ID, walletbalance, lastPaid, conanPlatformid, earnratemultiplier FROM accounts WHERE conanPlatformid = ?",
                        (platformID,))
                    ...
                    if account is not None:
                        ...
                        if lastPaid < (now - timedelta(minutes=paycheckinterval)):
                            dbCur.execute("UPDATE accounts SET walletbalance = ?, lastPaid = ? WHERE ID = ?",
                                          (walletbalance + paycheck, now, account[0]))
                            dbCon.commit()
