    ...
    while True:
        ...
        if enabledServers is not None:
            for enabledServer in enabledServers:
                ...
                dbCur.execute("DELETE FROM {server}_recent_pvp WHERE loadDate < NOW() - INTERVAL 15 MINUTE".format(
                    server=serverName))
                dbCon.commit()
                # age off wanted players
                dbCur.execute(
                    "UPDATE {server}_wanted_players SET wantedLevel =0, bounty=0 WHERE wantedLevel >0 AND lastseen < NOW() - INTERVAL 2 Day".format(
                        server=serverName))
                dbCon.commit()
                os.chdir("..\\WebPages")
                base_map = folium.Map(crs='Simple', min_zoom=1, max_zoom=3, zoom_start=2, tiles=None, location=[0, 0],
                                      control_scale=True)
                conan_overlay = folium.raster_layers.ImageOverlay(name='map', image='images\\conan.jpg',
                                                                  bounds=[[-370, -342], [445, 475]], zindex=1)
                ...
                try:
                    ...
                    dbCur.execute(
                        "SELECT player, x, y, killstreak, wantedlevel, bounty FROM {servername}_wanted_players".format(
                    ...
                    if rowcount != 0:
                        for user in users:
                            ...
                            if wantedlevel > 0:
                                ...
                                html = '''{player}<br/>Kill Streak:{killstreak}<br/>Wanted Level:{wantedlevel}<br/>Bounty:{bounty}'''.format(
                                    player=player, killstreak=killstreak, wantedlevel=wantedlevel, bounty=bounty)
                                popupdata = folium.Popup(html, min_width=100, max_width=500)

                                folium.Marker([mapY, mapX], popup=popupdata, icon=folium.DivIcon(
                                    html=f"""<div style="font-family: courier new; -webkit-text-fill-color: #ff00ff; -webkit-text-stroke-width: 2px; -webkit-text-stroke-color: black; font-size: large;"><img src="images\MASCOT_icon.png" width="50" height="50">{player}</center></div>""")).add_to(
                                    wantedPlayers)

                    recentpvp = folium.plugins.FeatureGroupSubGroup(mainFeatureGroup, name='Recent PVP')
                    dbCur.execute(
                        "SELECT id, pvpname, x, y, loadDate FROM {servername}_recent_pvp".format(servername=serverName))
                    pvps = dbCur.fetchall()
                    rowcount = len(pvps)
                    if rowcount != 0:
                        for pvp in pvps:
                            ...
                            mapX = int(x) / 1000
                            mapY = (int(y) / 1000) * -1
                            html = '''PVP Detected!<br/>{pvpname}<br/>detection time at:{pvpdatetime}'''.format(
                                pvpname=name, pvpdatetime=datetime)
                            popupdata = folium.Popup(html, min_width=100, max_width=500)
                            folium.Marker([mapY, mapX], popup=popupdata, icon=folium.DivIcon(
                                html=f"""<div style="font-family: courier new; -webkit-text-fill-color: #ff00ff; -webkit-text-stroke-width: 2px; -webkit-text-stroke-color: black; font-size: large;"><img src="images\pvpmarker.gif" width="50" height="50"></center></div>""")).add_to(
                                recentpvp)
