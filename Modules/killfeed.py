...
def get_server_info():
    connect_mariadb()
    mariaCur.execute("SELECT DatabaseLocation, lastEventTime FROM servers WHERE id = ?", (Server_ID,))
...
#                    user="Pythios",
#                    password="596^Cty3akL#49",
#                    host="135.148.136.196",
#                    port=4310,
#                    database="irrelevantecon"
#
#                    )
#
...
#                            try:
#                                mariaCur.execute("INSERT INTO exi_offenders (conanPlayerName, conanPlatformID, strikes, offenses) VALUES (?,?,1,1)", (player, PlayerPlatformID[0]))
#                                mariaCon.commit()
#                            except Exception as e:
#                                if "Duplicate" in str(e):
#                                    mariaCur.execute("SELECT strikes, offenses FROM exi_offenders WHERE conanPlatformID =?", (PlayerPlatformID[0], ))
#                                    info = mariaCur.fetchone()
#                                    strikes = info[0]
#                                    offenses = info[1]
#                                    newStrikes = int(strikes) + 1
#                                    newOffenses = int(offenses) + 1
#                                    mariaCur.execute("UPDATE exi_offenders SET strikes =?, offenses =? WHERE conanPlatformID =?", (newStrikes, newOffenses, PlayerPlatformID[0]))
#                                    mariaCon.commit()
#                                    pass
#                                else:
#                                    print(e)
...
#                        matchFound = 0
#                        mariaCur.execute("SELECT id, conanplatformid, conanplayer, killstreak, highestkillstreak, wantedLevel, bounty FROM exi_wanted_players")
...
#                                    mariaCur.execute("UPDATE exi_wanted_players SET killstreak =? WHERE id =?",(newkillstreak, playerid))
#                                    
#                                    if newkillstreak < 5:
#                                        newwantedlevel = 0
...
#                                        mariaCur.execute("UPDATE exi_wanted_players SET highestkillstreak = ? WHERE id =?",(newkillstreak, playerid))
#                                    mariaCur.execute("UPDATE exi_wanted_players SET wantedLevel =? WHERE id =?",(newwantedlevel, playerid))
#                                    mariaCur.execute("UPDATE exi_wanted_players SET bounty =? WHERE id =?",(newbounty, playerid))
...
#                                    mariaCon.commit()
#                                    #clear wanted
#                                    mariaCur.execute("UPDATE exi_wanted_players SET wantedLevel = '0' WHERE id =?",(playerid, ))
#                                    mariaCur.execute("UPDATE exi_wanted_players SET bounty = '0' WHERE id =?",(playerid, ))
#                                    mariaCur.execute("UPDATE exi_wanted_players SET killstreak = '0' WHERE id =?",(playerid, ))
#                                    mariaCon.commit()
#                                    matchFound = 1
#                            if matchFound == 0:
#                                print("no existing wanted players found")
#                                mariaCur.execute("INSERT INTO exi_wanted_players (conanplatformid, conanplayer, killstreak, highestkillstreak, wantedlevel, bounty) VALUES (?,?,'1','1','0','0')",(PlayerPlatformID[0],player))
#                                mariaCon.commit()
#                        else:
#                            print("no existing wanted players found")
#                            mariaCur.execute("INSERT INTO exi_wanted_players (conanplatformid, conanplayer, killstreak, highestkillstreak, wantedlevel, bounty) VALUES (?,?,'1','1','0','0')",(PlayerPlatformID[0],player))
...
#                    mariaCur.execute("INSERT INTO exi_recent_pvp (name, x, y, datetime) VALUES (?,?,?,?)",(pvpmarkername, int(KillLocationX), int(KillLocationY), now))
#                    mariaCon.commit()
#                except Exception as e:
...
#                    #get random insult
#                    insultConnection = sqlite3.connect('C:\\Game-Servers\\Conan-Event-System\\events.db')
#                    insultcursor = insultConnection.cursor()
#                    insult = insultcursor.execute("SELECT insult FROM insults ORDER BY RANDOM() LIMIT 1")
...
#                
#                
#                #record last event time to file
#                lastEventTimeSave = open("lastEventTimeSave.txt", "w+")
#                lastEventTimeSave.write(str(eventTime))
#            lastEventTimeSave.close()
#        
#        if list != '':
#            print (list)
#            channel = client.get_channel(816540520536866836)
